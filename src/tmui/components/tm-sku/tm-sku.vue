        ...
        <template v-slot:default>
            <view class="pa-24">
                <view class="flex flex-row flex-row-center-start">
                    <tm-image :preview="true" v-if="nowItemInfo" :width="120" :height="120"
                              :src="nowItemInfo.img"></tm-image>
                    <view class="pl-24">
                        <tm-statistic prefix="¥" :endVal="(nowItemInfo?.price??0)*nowInputNumber" :color="props.color"
                                      :font-size="40"
                                      :duration="500"/>
                        <tm-text color="grey" :font-size="24"
                                 :label="nowItemInfo&&nowItemInfo?.num>0?'有货'+' | 库存 '+nowItemInfo?.num:'无货'"></tm-text>
                ...
                <view v-if="_list">
                    <view class="" v-for="(item,index) in _list.data" :key="index">
                        ...
                        <tm-radio-group v-model="nowSelected[index]" v-if="item?.children" direction="row">
                            <tm-radio :disabled="!item2.num" :value="item2.id" v-for="(item2,index2) in item.children"
                                      :key="index2" :custom="false">
        ...
        <template v-slot:foot>
            <view class="mb-20 px-24">
                <view style="height:40rpx" class="flex flex-row flex-row-center-center">
                    ...
                    <tm-text v-if="nowInputNumber>maxCount&&nowInputNumber&&nowItemInfo" :color="props.color"
                             label="库存不足"></tm-text>
                </view>
                <view class="flex flex-row">
                    <view style="width:363rpx">
                        <tm-button @click="addGou" block :is-disabled-round-andriod="true" _class="round-l-24 round-r-0"
                                   ...
                                   label="加购物车" :height="80"></tm-button>
                    </view>
                    <view style="width:363rpx">
                        <tm-button @click="buyGou" block
